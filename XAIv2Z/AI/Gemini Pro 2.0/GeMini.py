...
# Потоковый вывод для LLaMa
def chat_stream(prompt):
    """
    Реализует потоковый вывод ответа от LLaMa.
...
def main():
    ...
    while True:
        ...
        if user_input.lower() == 'exit':
            ...
            threading.Thread(target=stop_dns_server).start()
            break
        
        # Добавляем запрос пользователя в историю диалога
        conversation_history.append({"role": "user", "content": user_input})
        
        print("[bold yellow]LLaMa[/bold yellow]: ", end=' ', flush=True)
